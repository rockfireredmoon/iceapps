...
/**
 * Centralises all mouse handling outside of the GUI. For example, keeps track of whether
 * camera drags are in action, if spatials are being dragged (and which ones), and if
 * spatials are being clicked.
 * <p>
 * Clients are expected to use {@link #addListener(org.icemoon.io.MouseManager.SceneQueueListener)}
 * and react to the various events.
 *
 */
public class MouseManager extends AbstractAppState implements AnalogListener, ActionListener {
    ...
    private final static List<String> MOVEMENT_MAPPINGS = Arrays.asList(MAPPING_UP, MAPPING_DOWN, MAPPING_LEFT, MAPPING_RIGHT);
    ...
    public interface Listener {
        ...
        void click(MouseManager manager, Spatial spatial, ModifierKeysAppState mods, int startModsMask, Vector3f contactPoint, CollisionResults results, float tpf, boolean repeat);
        ...
        void dragStart(Vector3f click3d, MouseManager manager, Spatial spatial, ModifierKeysAppState mods, Vector3f direction);

        void drag(MouseManager manager, Spatial spatial, ModifierKeysAppState mods, Vector3f click3d, Vector3f lastClick3d, float tpf, int startModsMask, CollisionResults results, Vector3f lookDir);
    }
    ...
    public MouseManager(Node rootNode, Alarm alarm) {
    ...
    public void setMode(Mode mode) {
        if (this.mode != mode) {
            ...
            if (mode.equals(Mode.PLACE)) {
                ...
                centerMark.setLocalTranslation(app.getContext().getSettings().getWidth() / 2, app.getContext().getSettings().getHeight() / 2, 0);
    ...
    public void initialize(AppStateManager stateManager, Application app) {
        ...
        if (mods == null) {
            throw new IllegalStateException(getClass().getName() + " requires " + ModifierKeysAppState.class.getName() + " to be attached.");
        }
    ...
    private void onNormalAnalog(String name, float value, float tpf) {
        ...
        if (draggingSpatial != null) {
            // Do a collision as well, but only to pass on to whatever is handling
            ...
            // Schedule repeat as well, if more dragging occurs this will get cancelled
            cancelRepeat();
            if (repeatDelay > 0) {
            ...
            if (!isDraggingCamera()) {
                ...
                if (spatialAtCursor != null && (hovering == null || !hovering.equals(spatialAtCursor))) {
                    ...
                    // Hovering over nothing, tell the previously informed listener we are no longer hovering
                    hovering = null;
    ...
    private void loadLocations() {
        ...
        dir = app.getCamera().getWorldCoordinates(new Vector2f(click2d.getX(), click2d.getY()), 1f).subtractLocal(click3d).normalizeLocal();
    }
        ...
        if (spatialAtCursor != null || draggingSpatial != null) {
            ...
            if (!isPressed) {
                ...
                if (draggingSpatial != null) {
                    ...
                    // Spatial clicked
                    targetListener.click(this, spatialAtCursor, mods, startModsMask, targetContactPoint, results, tpf, false);
                }
            ...
            if (isPressed) {
                ...
                // "Nothing" has been clicked on. Record the camera position to determine
                ...
                if (!hasCameraMoved()) {
                    // Click was released when "Nothing" was clicked on previously,
                    // and the camera hasn't moved, so this is a "default" select.                 

                    // Find where we collid, this may be useful in some contexts for a default select
...
//                rootNode.collideWith(ray, results);
    ...
    private void scheduleRepeat(float delay, final Listener listener) {
        repeatTask = alarm.timed(new Callable<Void>() {
            public Void call() throws Exception {
                // Spatial clicked
                listener.click(MouseManager.this, spatialAtCursor, mods, startModsMask, targetContactPoint, results, 0.1f, true);
    ...
    private void checkSpatialAtCursor(Action action) {
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            /* Ask all of the listeners if they are interested in the Collision, they
             * can either signal definite interest, or as fallback if no other listener
             * handles it.
             */
