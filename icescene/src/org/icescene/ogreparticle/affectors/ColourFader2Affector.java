...
public class ColourFader2Affector extends AbstractOGREParticleAffector {
    ...
    public void update(ParticleData p, float tpf) {
        ...
        if (aliveFor > stateChange) {
            p.color.set(p.color.r + (adjustment2.x * tpf),
                    p.color.g + (adjustment2.y * tpf),
                    p.color.b + (adjustment2.w * tpf),
                    p.color.a + (adjustment2.z * tpf)).clamp();
        } else {
            p.color.set(p.color.r + (adjustment1.x * tpf),
                    p.color.g + (adjustment1.y * tpf),
                    p.color.b + (adjustment1.z * tpf),
                    p.color.a + (adjustment1.w * tpf)).clamp();
        }
    }
