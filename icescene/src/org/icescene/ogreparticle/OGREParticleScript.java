...
//    private Vector3f commonUpVector = new Vector3f(0, 1, 0);
    ...
    public OGREParticleScript(String name, OGREParticleConfiguration configuration, EmitterFactory emitterFactory, AffectorFactory affectorFactory) {
        this.name = name;
    ...
    public void write(OutputStream fos, boolean b) throws IOException {
        ...
        if (getCommonDirection() != null) {
            pw.println(String.format("\tcommon_direction %s", OGREParticleConfiguration.formatForWrite(getCommonDirection())));
        }
        if (getCommonUpVector() != null) {
            pw.println(String.format("\tcommon_up_vector %s", OGREParticleConfiguration.formatForWrite(getCommonUpVector())));
        }
        pw.println(String.format("\tlocal_space %s", isLocalSpace()));
        pw.flush();
