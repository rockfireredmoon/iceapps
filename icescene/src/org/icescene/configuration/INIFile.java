...
public final class INIFile {
	 ...
	 */
	public INIFile(String pstrPathAndName) throws IOException {
		this.mpropEnv = getEnvVars();
		this.mhmapSections = new LinkedHashMap();
	...
	public void load(String pstrPathAndName) throws IOException {
		this.mstrFile = pstrPathAndName;
		// Load the specified INI file.
		if (checkFile(pstrPathAndName)) {
			loadFile();
	...
	public void load(InputStream in) throws IOException {
		...
		try {
			objFRdr = new InputStreamReader(in);
			if (objFRdr != null) {
				objBRdr = new BufferedReader(objFRdr);
				if (objBRdr != null) {
					while ((strLine = objBRdr.readLine()) != null) {
						iPos = -1;
						line++;
						final String trim = strLine.trim();
						if (strLine.length() == 0) {
							...
							objSec = null;
							strSection = trim.substring(1, trim.length() - 1);
							...
							if (trim.length() != 0) {
								if(objSec == null) 
									throw new IOException(String.format("Invalid format on line %d '%s'.", line, trim));
								objSec.setProperty(trim, "", strRemarks);
							}
							strRemarks = null;
						}
					}
	...
	/**
	 * Sets the specified float property.
	 *
	 * @param pstrSection
	 *            the INI section name.
	 * @param pstrProp
	 ...
	 */
	public void setFloatProperty(String pstrSection, String pstrProp, float pfltVal, String pstrComments) {
		INISection objSec = null;
		...
		objSec.setProperty(pstrProp, Float.toString(pfltVal), pstrComments);
	}
	...
	public boolean hasSection(String section) {
		return mhmapSections.containsKey(section);
	}
	...
	/**
	 * Flush changes back to an output stream.
	 */
	public boolean save(OutputStream out) {
		...
		try {
			...
			objWriter = new OutputStreamWriter(out);
			itrSec = this.mhmapSections.keySet().iterator();
			while (itrSec.hasNext()) {
				strName = (String) itrSec.next();
				objSec = (INISection) this.mhmapSections.get(strName);
	...
	private void loadFile() throws IOException {
		FileInputStream in = new FileInputStream(this.mstrFile);
		try {
			load(in);
		} finally {
			in.close();
