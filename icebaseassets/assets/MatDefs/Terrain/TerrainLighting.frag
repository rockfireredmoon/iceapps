  ...
  vec4 calculateDiffuseBlend(in vec2 texCoord) {
    ...
    diffuseColor *= 1;
      ...
      diffuseColor = mix( diffuseColor, diffuseColor1, alphaBlend.r );
        ...
        diffuseColor = mix( diffuseColor, diffuseColor2, alphaBlend.g );
        #ifdef DIFFUSEMAP_3
          vec4 diffuseColor3 = texture2D(m_DiffuseMap_3, texCoord * m_DiffuseMap_3_scale);
          diffuseColor = mix( diffuseColor, diffuseColor3, alphaBlend.b );
        #endif
      #endif
    #endif
    return diffuseColor;
  ...
  vec3 calculateNormal(in vec2 texCoord) {
    ...
    vec4 calculateTriPlanarDiffuseBlend(in vec3 wNorm, in vec4 wVert, in vec2 texCoord) {
        ...
        vec4 diffuseColor = tex0 * 1;
          ...
          #ifdef DIFFUSEMAP_2
            diffuseColor = mix( diffuseColor, tex2, alphaBlend.g );
            #ifdef DIFFUSEMAP_3
              diffuseColor = mix( diffuseColor, tex3, alphaBlend.b );
            #endif
          #endif
        #endif
