...
void main(){
   ...
   float dist = distance(g_CameraPosition.xz, worldPos.xz);
       ...
       #ifdef INVERT
           modelSpacePos.xz += 0.1*m_Wind*(1.0 - inTexCoord.y)*sin(angle);
       #else
           modelSpacePos.xz += 0.1*m_Wind*inTexCoord.y*sin(angle);
       #endif
   #endif
   ...
   #ifdef FADE_ENABLED
   texCoordZ = clamp((m_FadeEnd - dist)/(m_FadeRange),0.0,1.0);
   #endif
