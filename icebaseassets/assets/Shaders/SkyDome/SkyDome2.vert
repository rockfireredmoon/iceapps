...
uniform float m_CycleSpeed;
	...
	uniform float m_CelestialBodyStart;
    uniform float m_CelestialBodySpeed;
    uniform float m_CelestialBodyDirection;
...
void main(){
    ...
    #ifdef HAS_CELESTIAL_BODY
        celestialBodyCoord = inTexCoord;
        float time = ((g_Time + m_CelestialBodyStart) *(m_CelestialBodySpeed*0.1));
        celestialBodyCoord.x += (time*sin((m_CelestialBodyDirection*(pi/180.0))));
        celestialBodyCoord.y += (time*cos((m_CelestialBodyDirection*(pi/180.0))));
    #endif
